...
```bash
pt_issue274$ make build
pt_issue274$ make deploy.guided
```
...
```bash
pt_issue274$ make run
pt_issue274$ curl http://localhost:3000/hello
```
...
```bash
pt_issue274$ sam logs -n HelloWorldFunction --stack-name <Name-of-your-deployed-stack> --tail
```
...
```bash
pt_issue274$ make dev
pt_issue274$ make test
```
...
```bash
pt_issue274$ aws cloudformation delete-stack --stack-name pt_issue274
```
...
```bash
pt_issue274$ pipenv lock -r > hello_world/requirements.txt
```
